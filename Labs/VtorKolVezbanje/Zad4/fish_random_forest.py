...
if __name__ == "__main__":
    ...
    dataset_2 = []
    for row in dataset:
        row_2 = [row[i] for i in range(len(row)) if i != col_index]
        dataset_2.append(row_2)

    dataset = dataset_2

    trainset = dataset[:int(len(dataset) * 0.85)]
    trainset_x = [row[:-1] for row in trainset]
    trainset_y = [row[-1] for row in trainset]

    testset = dataset[int(len(dataset) * 0.85):]
    testset_x = [row[:-1] for row in testset]
    testset_y = [row[-1] for row in testset]

    classifier = RandomForestClassifier(criterion=crt, n_estimators=trees, random_state=0)
    classifier.fit(trainset_x, trainset_y)

    accuracy = 0

    for true, pred in zip(classifier.predict(testset_x), testset_y):
        if true == pred:
            accuracy += 1

    accuracy /= len(testset_x)
    ...
    predicted_class_proba = classifier.predict_proba([entry])[0]
    print(f'{predicted_class_proba}')
