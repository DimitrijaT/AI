...
class GhostOnSkates(Problem):
    ...
    def check_valid(state, walls, n):
        # (state[0] == x)  (state[1] == y)
        # n = 4
        return state[0] < n and state[1] < n and (state[0], state[1]) not in walls
    ...
    def successor(self, state):
        ...
        for i in range(1, 4):
            new_y = ghost_y + i
            if self.check_valid(state, self.walls, self.n):
                successors[f"Gore {i}"] = (ghost_x, new_y)

        # RIGHT
        for i in range(1, 4):
            new_x = ghost_x + i
            if self.check_valid(state, self.walls, self.n):
                successors[f"Desno {i}"] = (new_x, ghost_y)
    ...
    def h(self, node):
        ...
        mnh = abs(state[0] - self.n - 1) + abs(state[1] - self.n - 1)
        mnh /= 3
