...
if __name__ == '__main__':
    ...
    col_index_to_remove = int(input())

    dataset_v2 = []
    for row in dataset:
        row_v2 = [row[el_i] for el_i in range(len(row)) if el_i != col_index_to_remove]
        dataset_v2.append(row_v2)

    dataset = dataset_v2

    num_trees = int(input())
    criterion_to_use = input()
    ...
    classifier = RandomForestClassifier(n_estimators=num_trees, criterion=criterion_to_use, random_state=0)
    ...
    # entry.remove(entry[col_index_to_remove])
    entry = [entry[i] for i in range(len(entry)) if i != col_index_to_remove]
    predicted_class = classifier.predict([entry])[0]
    print(predicted_class)
    predicted_class_probabilities = classifier.predict_proba([entry])[0]
