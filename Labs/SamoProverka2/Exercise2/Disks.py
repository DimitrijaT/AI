...
class Disks(Problem):
    ...
    def successor(self, state):
        ...
        for disk in range(len(state)):
            if state[disk] != 0:
                # state[disk] != 0
                if disk + 1 < len(state) and state[disk + 1] == 0:
                    new_state = state.copy()
                    new_state[disk + 1] = state[disk]
                    new_state[disk] = 0
                    successors[f"D1: Disk {state[disk]}"] = tuple(new_state)

                if disk + 2 < len(state) and state[disk + 2] == 0 and state[disk + 1] != 0:
                    new_state = state.copy()
                    new_state[disk + 2] = state[disk]
                    new_state[disk] = 0
                    successors[f"D2: Disk {state[disk]}"] = tuple(new_state)

                if disk - 1 >= 0 and state[disk - 1] == 0:
                    new_state = state.copy()
                    new_state[disk - 1] = state[disk]
                    new_state[disk] = 0
                    successors[f"L1: Disk {state[disk]}"] = tuple(new_state)

                if disk - 2 >= 0 and state[disk - 2] == 0 and state[disk - 1] != 0:
                    new_state = state.copy()
                    new_state[disk - 2] = state[disk]
                    new_state[disk] = 0
                    successors[f"L2: Disk {state[disk]}"] = tuple(new_state)
    ...
    def h(self, node):
        ...
        state = node
        counter = 0
        for x, y in zip(init_state, goal_state):
            if x != y and x != 0:
                counter += 1
        return counter
...
if __name__ == "__main__":
    number_disks_n = int(input())
    disk_list_length = int(input())
    startState = []
    endState = []
    for i in range(0, number_disks_n):
        startState.append(i + 1)
    for i in range(number_disks_n, disk_list_length):
        startState.append(0)
    ...
    disks = Disks(tuple(startState), tuple(endState))
