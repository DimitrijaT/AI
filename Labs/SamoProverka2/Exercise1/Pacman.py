...
def ProdolzhiPravo(x, y, d, obstacles):
    if d == "istok":
        if x + 1 <= 9 and [x + 1, y] not in obstacles:
            x += 1
    elif d == "zapad":
        if x - 1 >= 0 and [x - 1, y] not in obstacles:
            x -= 1
    elif d == "sever":
        if y + 1 <= 9 and [x, y + 1] not in obstacles:
            y += 1
    elif d == "jug":
...
def ProdolzhiNazad(x, y, d, obstacles):
    if d == "istok":
        if x - 1 >= 0 and [x - 1, y] not in obstacles:
            x -= 1
            d = "zapad"
...
def SvrtiDesno(x, y, d, obstacles):
    if d == "istok":
        if y - 1 >= 0 and [x, y - 1] not in obstacles:
            y -= 1
            d = "jug"
...
class Pacman(Problem):
    ...
    def successor(self, state):
        # (x, y, direction, ((food_x,food_y),...))
        successors = dict()

        pacman_x, pacman_y, pacman_direction = state[0], state[1], state[2]
        foods = state[3]

        new_x, new_y, new_d = ProdolzhiPravo(pacman_x, pacman_y, pacman_direction, obstacles)
        if [new_x, new_y] != [pacman_x, pacman_y]:
            successors["ProdolzhiPravo"] = (
                new_x, new_y, new_d, tuple([f for f in foods if (f[0], f[1]) != (new_x, new_y)]))

        new_x, new_y, new_d = ProdolzhiNazad(pacman_x, pacman_y, pacman_direction, obstacles)
        if [new_x, new_y] != [pacman_x, pacman_y]:
            successors["ProdolzhiNazad"] = (
                new_x, new_y, new_d, tuple([f for f in foods if (f[0], f[1]) != (new_x, new_y)]))

        new_x, new_y, new_d = SvrtiLevo(pacman_x, pacman_y, pacman_direction, obstacles)
        if [new_x, new_y] != [pacman_x, pacman_y]:
            successors["SvrtiLevo"] = (
                new_x, new_y, new_d, tuple([f for f in foods if (f[0], f[1]) != (new_x, new_y)]))

        new_x, new_y, new_d = SvrtiDesno(pacman_x, pacman_y, pacman_direction, obstacles)
        if [new_x, new_y] != [pacman_x, pacman_y]:
    ...
    def h(self, node):
        ...
        minmnh = 0
        if len(stars) != 0:
            minmnh = abs(pacman_x - stars[0][0]) + abs(pacman_y - stars[0][1])

            for x, y in stars:
                minmnh = max(abs(pacman_x - x) + abs(pacman_y - y), minmnh)
...
if __name__ == "__main__":
    ...
    pacman_x = int(input())
    pacman_y = int(input())
    pacman_direction = input().lower()
    food = []
    food_count = int(input())
    for i in range(food_count):
        food_read = input().split(",")
        food.append((int(food_read[0]), int(food_read[1])))

    food = tuple(food)

    # pacman = Pacman(obstacles, (pacman[0], pacman[1], pacman[2], food))
    pacman = Pacman(obstacles, (pacman_x, pacman_y, pacman_direction, food))
