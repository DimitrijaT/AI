...
# SEVER
def north(snake_head, snake_body, obstacles):
    if (snake_head[1] + 1) < 10 and (snake_head[0], snake_head[1] + 1) not in snake_body and\
            (snake_head[0], (snake_head[1] + 1)) not in obstacles:
        snake_body.pop()
...
# ZAPAD
def west(snake_head, snake_body, obstacles):
    if (snake_head[0] - 1) >= 0 and\
            (snake_head[0] - 1, snake_head[1]) not in snake_body and\
            ((snake_head[0] - 1), snake_head[1]) not in obstacles:
        snake_body.pop()
        snake_body.insert(0, (snake_head[0], snake_head[1]))
...
# JUG
def south(snake_head, snake_body, obstacles):
    if (snake_head[1] - 1) >= 0 and\
            (snake_head[0], snake_head[1] - 1) not in snake_body and\
            (snake_head[0], (snake_head[1] - 1)) not in obstacles:
        snake_body.pop()
        snake_body.insert(0, (snake_head[0], snake_head[1]))
...
# ISTOK
def east(snake_head, snake_body, obstacles):

    # if snake_head[0] == 2 and snake_head[1] == 3:
    #     print("DOBAR DEN")

    if (snake_head[0] + 1) < 10 and\
            (snake_head[0] + 1, snake_head[1]) not in snake_body and\
            ((snake_head[0] + 1), snake_head[1]) not in obstacles:
        snake_body.pop()
        snake_body.insert(0, (snake_head[0], snake_head[1]))
...
def ProdolzhiPravo(snake_head, snake_body, obstacles):
    if snake_head[2] == "east":
        return east(snake_head, snake_body, obstacles)
    elif snake_head[2] == "west":
        return west(snake_head, snake_body, obstacles)
    elif snake_head[2] == "north":
        return north(snake_head, snake_body, obstacles)
    elif snake_head[2] == "south":
        return south(snake_head, snake_body, obstacles)


def SvrtiDesno(snake_head, snake_body, obstacles):
    if snake_head[2] == "east":
        return south(snake_head, snake_body, obstacles)
    elif snake_head[2] == "west":
        return north(snake_head, snake_body, obstacles)
...
def SvrtiLevo(snake_head, snake_body, obstacles):
    if snake_head[2] == "east":
        return north(snake_head, snake_body, obstacles)
    elif snake_head[2] == "west":
        return south(snake_head, snake_body, obstacles)
...
class Snake(Problem):
    ...
    def successor(self, state):
        ...
        new_head, new_body = ProdolzhiPravo(list(state[0]), list(state[1]), list(self.obstacles))
        ...
        new_head, new_body = SvrtiDesno(list(state[0]), list(state[1]), list(self.obstacles))
        if [new_head[0], new_head[1]] != [snake_x, snake_y]:
            food_len = len(green_apples)
            new_food_state = tuple([f for f in green_apples if (f[0], f[1]) != (new_head[0], new_head[1])])
            if food_len != len(new_food_state):
        ...
        new_head, new_body = SvrtiLevo(list(state[0]), list(state[1]), list(self.obstacles))
        if [new_head[0], new_head[1]] != [snake_x, snake_y]:
            food_len = len(green_apples)
            new_food_state = tuple([f for f in green_apples if (f[0], f[1]) != (new_head[0], new_head[1])])
            if food_len != len(new_food_state):
...
if __name__ == '__main__':
    ...
    for i in range(red_num):
        red_apple = input().split(",")
        red_apples.append((int(red_apple[0]), int(red_apple[1])))

    snakeHead = (0, 7, "south")
    snakeBody1 = (0, 8)
    snakeBody2 = (0, 9)

    snake = Snake((snakeHead, (snakeBody1, snakeBody2), tuple(green_apples)), tuple(red_apples))
